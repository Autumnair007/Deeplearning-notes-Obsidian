...
**公式解释：** 这是一条流程式，不是矩阵计算。它依次要求确定视觉粒度、把视觉与文本投到可比较的特征维、计算对应分数，再把展平位置恢复为二维预测；式中没有求和，也没有消去维度。
...
**公式解释：** $C$ 是候选类别数，$D_t$ 是文本编码器输出维度；$T[c,d]$ 是类别 $c$ 文本向量的第 $d$ 个分量。这里只声明文本输入 shape，没有发生运算或维度消去。
...
**公式解释：** 映射 $\psi$ 对每个类别文本向量独立作用，把最后一维从 $D_t$ 变为视觉维 $D_v$，类别维 $C$ 保留。输出 $T'[c,:]$ 是能与视觉特征直接点积的第 $c$ 类语义锚点；改变的是坐标空间，不是类别数量。
...
**公式解释：** 归一化在最后一个特征维 $D$ 上进行。对 $P=[B,N,D]$，每个 patch 的 $D$ 维向量除以自己的范数；对 $T=[C,D]$，每个类别文本也独立归一化。输出 shape 不变，没有消去维度；$\hat P[b,n,:]$ 与 $\hat T[c,:]$ 都是单位向量，后续点积只比较方向而不受模长影响。
...
**公式解释：** 这一步让每个视觉补丁和全部类别文本做余弦相似度比较。$\hat P=[B,N,D]$，$\hat T^T=[D,C]$；矩阵乘法消去共同特征维 $D$，留下 `[B,N,C]`。$S[b,n,c]$ 表示第 $b$ 张图第 $n$ 个补丁与第 $c$ 个类别文本的匹配分数。
...
**公式解释：** 这是上一式的具体 shape 代入：每张图有 400 个 patch，每个 patch 用 512 维向量与 20 个类别文本比较；512 在矩阵乘法中被消去，输出为 400 个位置各自的 20 类分数。$S[0,17,3]$ 就是第 1 张图第 18 个 patch 对第 4 类的匹配分数。
...
**公式解释：** 输入 $S=[B,N,C]$，先 `permute` 为 `[B,C,N]`，再用 $N=H'W'$ 把最后一维拆成二维网格，得到 `[B,C,H',W']`。两步只重排索引，不做乘法或求和，因此没有维度被消去；$S_{\mathrm{map}}[b,c,h,w]$ 与原序列对应 patch 的 $S[b,n,c]$ 是同一个数。
...
**公式解释：** 输入是低分辨率类别图 `[B,C,H',W']`，上采样只把空间维变为目标 $H,W$，批量和类别维保持不变。输出 $M[b,c,h,w]$ 表示输入图像坐标 $(h,w)$ 的类别 $c$ 分数；双线性插值用邻近低分辨率位置加权生成该值，不消去任何维度。
...
**公式解释：** 图像 $I$ 被编码为全局向量 $V_g\in\mathbb R^D$，$C$ 个类别文本分别成为 $t_c\in\mathbb R^D$；余弦相似度在特征维 $D$ 上点积并消去它，每个类别只得到一个标量 $s_c$。这个标量没有空间维，因此 Grad-CAM 再求 $s_c$ 对某层二维视觉特征的梯度，把类别判断还原为热图。哪些位置变化会明显影响 $s_c$，哪些位置就更可能与该类别有关。
...
**公式解释：** $P[b,i,j,:]$ 是网格位置 $(i,j)$ 的 $D$ 维 patch 向量，$T[c,:]$ 是类别 $c$ 的 $D$ 维文本向量。两者共享可比较的特征维 $D$；该式只声明输入 shape，还没有进行相似度计算或消去维度。
...
**公式解释：** 对每个 $b,i,j$，用 $P[b,i,j,:]$ 与全部 $T[c,:]$ 在特征维 $D$ 上计算余弦相似度，特征维被消去，类别维 $C$ 被保留。`Norm` 再按论文指定的维度缩放分数，输出仍是 `[B,H',W',C]`；$M_t[b,i,j,c]$ 表示该 patch 与类别 $c$ 的归一化匹配程度。不同论文的归一化轴可能不同，所以必须结合代码确认。
...
**公式解释：** $R$ 是每张图的候选区域数；$s[b,r,i,j]$ 表示第 $r$ 个软掩码在位置 $(i,j)$ 的归属权重。该式只声明 `[B,R,H',W']` 掩码张量，没有求和；区域维和空间维都保留。
...
**公式解释：** $F[b,i,j,:]$ 是位置 $(i,j)$ 的 $D$ 维视觉特征。它与掩码 $s$ 共享 $B,H',W'$，因此可在相同位置逐元素加权；这里仍只是输入接口，没有维度消去。
...
**公式解释：** 分子把区域权重 $s_{r,i,j}$ 广播到特征维，逐位置乘上 $F_{i,j}\in\mathbb R^D$，再沿 $i,j$ 两个空间维求和；空间维被消去，只剩 $D$ 维向量。分母是该软掩码的有效面积，广播除法避免大区域仅因像素更多而模长更大。输出 $z_r[d]$ 是区域 $r$ 的第 $d$ 个平均特征分量，$\epsilon$ 防止空掩码除零。
...
**公式解释：** $z_r,t_c$ 都是 $D$ 维向量，余弦相似度在特征维 $D$ 上计算并消去它，输出一个标量 $S_{r,c}$。它表示“第 $r$ 个候选区域像不像类别 $c$”；此时分类发生在区域层面，区域内部共享同一个语义判断。
...
**公式解释：** 对固定类别 $c$ 和像素 $(i,j)$，把每个区域的类别分数 $S_{r,c}$ 乘以该像素对区域的归属权重 $s_{r,i,j}$，再沿区域维 $R$ 求和并消去它。输出 $Y_{c,i,j}$ 是所有候选区域对该像素类别 $c$ 的加权投票；多个软掩码可重叠，所以一个像素可能同时接收多个区域贡献。
...
**公式解释：** $M_c=[H',W']$ 是类别 $c$ 的 CAM，$F_I=[H',W',D]$ 是图像语义对齐模块输出。$M_c$ 在特征维广播，与 $F_I$ 逐位置相乘；MAP 再沿 $H',W'$ 求加权平均，消去两个空间维，输出 $f_{\mathrm{image}}\in\mathbb R^D$。其第 $d$ 个元素是当前图像类别 $c$ 前景区域的平均第 $d$ 维视觉特征，而不是单个像素。收集许多图像的摘要并做 K-means，聚类中心形成视觉原型。
...
**公式解释：** 设 $f_i=[D]$，文本原型矩阵 $P^T=[K,D]$，转置后为 `[D,K]`。矩阵乘法在特征维 $D$ 上点积并消去它，得到 $K$ 个原型分数 $p_i=[K]$；$p_i[k]$ 表示视觉表示 $i$ 与第 $k$ 个文本原型的温度缩放相似度。除以 $\tau$ 控制分布尖锐度，训练时同类原型为正目标、其他原型为负目标。
...
**公式解释：** 若文本 $t\in\mathbb R^{D_t}$，第一层 $W_a^Tt+b_a$ 把 $D_t$ 投影到隐藏维 $D_h$，`tanh` 逐元素作用而不改变 shape；第二层 $W_b^T$ 再把 $D_h$ 投影到 DINOv2 视觉维 $D_v$，加 $b_b$ 后输出 $\psi(t)\in\mathbb R^{D_v}$。矩阵乘法分别消去输入特征维 $D_t$ 和隐藏维 $D_h$；输出每个元素是映射后文本在 DINO 坐标系中的一个视觉语义分量。
...
**公式解释：** $v_{A_i},\psi(t)$ 都是 $D_v$ 维向量，余弦相似度先消去特征维，为每个注意力头/候选区域 $i$ 得到一个标量；`max_i` 再消去候选维 $i$，输出单个图文匹配分数。它表示所有候选区域中最像文本的那一个，只要一个区域高度匹配，整对图文就可获得高分。
...
**公式解释：** $Q[b,n,:]$ 是第 $b$ 张图第 $n$ 个 patch 的 $D$ 维查询向量。这里只声明 `[B,N,D]` 接口，批量、位置和特征维均保留，没有发生求和。
...
**公式解释：** $U$ 是缓存条目数，$K[u,:]\in\mathbb R^D$ 是第 $u$ 个视觉键，$V[u,:]\in\mathbb R^C$ 是它绑定的类别证据。键与查询共享特征维 $D$，值把每个键映射到 $C$ 个类别；该式只声明 shape。
...
**公式解释：** 第一步 `Q @ K^T` 用 `[B,N,D] @ [D,U]` 消去特征维 $D$，得到 `[B,N,U]` 的查询—缓存相似度；$\sigma$ 逐元素变换权重，shape 不变。第二次乘 `V=[U,C]` 时消去缓存维 $U$，输出 `[B,N,C]`；`Norm` 只缩放数值。$M_{\mathrm{cache}}[b,n,c]$ 是所有缓存条目对 patch $n$ 属于类别 $c$ 的加权投票。
...
**公式解释：** 这不是数值运算式，而是跨模态对齐的三项检查：视觉粒度决定一行预测代表全图、patch 还是区域；特征空间决定两端能否直接点积；语义锚点决定比较对象是类别文本、文本原型还是视觉缓存。三项中任一项定义不清，相似度公式即使 shape 正确也可能没有可靠语义。
