...
| **疑问点**               | **核心机制**  | **解答**                                                                          |
| --------------------- | --------- | ------------------------------------------------------------------------------- |
| **可学习查询 $Q$**         | **语义意图**  | **是模型权重的一部分，存储在模型中。** 它们是一组**固定的、可训练的向量**。                                      |
| **$Q$ 的数量**           | **超参数设置** | $Q$ 的数量（例如 100 个）是**可设置的超参数**，代表模型**在一张图上能够预测的实例上限**。                           |
| **$Q$ 的内容**           | **类别无关**  | $Q$ **不绑定特定类别**。训练促使它们学会充当**通用的实例特征提取器**，负责定位图像中的任意一个对象。                        |
| **交叉注意力中的 $Q, K, V$** | **信息流**   | **$Q$ (查询)** 来源于**可学习查询向量**（模型权重）（解码器）。**$K/V$ (键/值)** 来源于**编码器输出的密集嵌入**（图像特征）。 |

**上下文向量 (Mask Embeddings)**

| **疑问点**       | **核心机制**   | **解答**                                                                      |
| ------------- | ---------- | --------------------------------------------------------------------------- |
| **上下文向量 $C$** | **解码器输出**  | 指 $Q$ 向量通过交叉注意力查询 $K/V$ 后生成的**新向量**（Mask Embeddings）。                       |
| **功能**        | **实例特征浓缩** | $C$ 是一个**高度精炼**的向量，它只编码了与该查询 $Q_i$ 意图相符的**特定实例**（例如，某一只狗）在图像中的**特征和上下文信息**。 |
| **最终任务**      | **掩码生成**   | $C$ 被送入预测头，**指导**模型将这个实例级特征投射回像素空间，生成该实例的**精确分割掩码**。                        |
|               |            |                                                                             |

我们之前的讨论是关于 **“基于查询的分割”** ，这是对您笔记中交叉注意力” 概念的推广应用。您的笔记已经为这个推广打下了坚实的基础。

| **概念**        | **笔记中的基础定义**                            | **在分割任务中的推广/应用**                                                         |
| ------------- | --------------------------------------- | ------------------------------------------------------------------------ |
| **查询 Q**      | 来自**解码器**的**隐藏状态 $s_t$**（动态、上下文相关）。     | **可学习查询向量**（固定数量、独立于图像的**模型权重**）。它代表**抽象的语义意图/实例占位符**。                   |
| **键 K / 值 V** | 来自**编码器**的**隐藏状态 $h_i$**（包含输入序列的上下文信息）。 | **密集视觉嵌入**（编码器输出的 Patch Token 序列）。它代表**图像的完整视觉上下文**。                     |
| **上下文向量 C**   | $C = \sum \alpha_i V_i$（**加权平均**，浓缩信息）。 | **Mask Embeddings**（或 $C$）。它是 $Q$ 从 $K/V$ 中提取的**特定实例特征**，用于指导最终的像素级掩码预测。 |
