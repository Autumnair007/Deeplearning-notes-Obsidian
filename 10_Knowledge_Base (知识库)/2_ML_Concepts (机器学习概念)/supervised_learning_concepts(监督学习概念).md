...
弱监督学习是另一个应对“标签困境”的重要范式，它关注的是**标签的质量而非数量**。

*   **核心思想**：利用那些不完全精确、含有噪声或只有部分信息的“弱标签”来训练模型。这些标签虽然不完美，但获取成本远低于强监督学习中的精确标签。
*   **数据形式**：弱监督主要有三种类型：
    *   **不精确监督 (Inexact Supervision)**：标签的粒度比我们期望的要粗。例如，我们想训练一个物体检测模型来框出图片中的猫，但只拥有图片级别的标签“有猫”，而没有具体的位置信息。
    *   **不准确监督 (Inaccurate Supervision)**：标签本身可能就是错的。例如，通过众包或自动化程序得到的标签，其中一部分可能存在错误。
    *   **不完整监督 (Incomplete Supervision)**：只有一小部分数据有标签。这种情况与半监督学习非常相似，两者的界限有时比较模糊。
*   **学习目标**：设计能够容忍甚至利用这些不完美标签的算法，从而在降低标注成本的同时，依然能训练出性能可靠的模型。
*   **优点**：极大地降低了对高质量人工标注的依赖，使得在很多现实场景中（如医疗、工业质检）进行大规模模型训练成为可能。
*   **与监督学习的关系**：如果说强监督学习是“精教”，那么弱监督学习就是“粗养”。它放宽了对“标准答案”的要求。
...
| **弱监督学习** | 大量**弱标签**数据（不精确、不准确等） | 从不完美的监督信号中学习模型 |
