...
ReME利用VLM的内在能力和聚合效应——虽然它们可能无法针对单个实例产生令人满意的结果，但VLM在集体层面分析数据时表现出一定程度的有效性。ReME的数据管道仅以图像作为输入，首先通过VLM配对的图像片段和文本标签构建一个语义丰富的 базовый (base) 集合。通过观察同模态数据特征中卓越的判别性，ReME利用它们的集体模式重新校准VLM引入的跨模态错位，生成一个质量增强的参考集。在推理时，我们对测试图像应用基本的类别无关分割算法，然后通过参照我们的参考集，使用简单的基于相似度的策略，对建议的掩膜进行对齐和聚合，从而得到最终预测。
