...
这篇论文提出了一种新的基于Transformer的框架，用于学习类别特定的目标定位图（class-specific object localization maps），作为弱监督语义分割（WSSS）的伪标签。作者受到以下事实的启发：标准Vision Transformer（ViT）中，一个类别令牌（one-class token）的注意力区域可以被用来形成类别无关的定位图。在此基础上，作者研究了Transformer模型是否也能通过学习多个类别令牌来有效地捕获类别特异性注意力，从而实现更具判别性的目标定位。为此，他们提出了一种多类令牌Transformer，简称MCTformer，它使用多个类别令牌来学习类别令牌与图像块令牌（patch tokens）之间的交互。

所提出的MCTformer可以成功地从对应于不同类别令牌的“类别到图像块注意力”（class-to-patch attentions）中生成具有类别判别性的目标定位图。此外，论文还提出利用从“图像块到图像块Transformer注意力”（patch-to-patch transformer attention）中提取的图像块级成对亲和性（patch-level pairwise affinity）来进一步优化定位图。更重要的是，该框架被证明能够完全补充类激活映射（CAM）方法，从而在PASCAL VOC和MS COCO数据集上取得了显著优越的WSSS结果。这些结果凸显了类别令牌对于WSSS的重要性。
...
值得注意的是，学习到的“图像块到图像块注意力”（patch-to-patch attention），作为训练的副产品且无需额外计算，可以作为图像块级成对亲和性（patch-level pairwise affinity）。这可以进一步优化类别特定的Transformer注意力图，显著提高定位性能。此外，作者还展示了所提出的Transformer框架在应用于图像块令牌时，能够完全补充CAM方法（通过同时学习基于类别令牌和图像块令牌表示的分类）。这导致类别令牌和图像块令牌之间高度一致，从而大大增强了其派生目标定位图的判别能力。
...
<br>
![图2](images/image_02.png)
<br>
**图2**展示了所提出的多类令牌Transformer (MCTformer-V1) 的整体架构。(a) 输入一张RGB图像。(b) 将图像分割成图像块，并转换为图像块令牌（Patch tokens），并加入位置编码（PE）。(c) 额外引入 $C$ 个可学习的类别令牌（extra learnable class tokens），其中 $C$ 是类别数量。这些类别令牌与图像块令牌拼接，形成Transformer编码器的输入。(d) 输入令牌经过 $L$ 个Transformer编码层。每个编码层包含多头注意力（Multi-Head Attention, MHA）模块和MLP模块，外加LayerNorm。在最后，输出的 $C$ 个类别令牌通过平均池化（Average Pooling）生成类别分数（Class scores），用于分类预测。(e) 从Transformer编码器中，可以聚合最后 $K$ 层的注意力图。从“类别到图像块注意力”中提取类别特定的注意力图 (Class-specific transformer attention maps)。(f) 从“图像块到图像块注意力”中提取图像块级的成对亲和性 (Patch-level pairwise affinity)。(g) 利用图像块级成对亲和性来优化类别特定的注意力图，生成细化的类别特定Transformer注意力图 (Refined class-specific transformer attention maps)，作为最终的目标定位图。
...
$$
A_{mct}^{\text{ref}}(c, i, j) = \sum_{k=1}^{N} \sum_{l=1}^{N} \hat{A}_{p2p}(i, j, k, l) \cdot A_{mct}(c, k, l)
$$
...
$$
y(c) = \frac{1}{D} \sum_{j=1}^{D} T_{cls}(c, j)
$$
...
<br>
![图3](images/image_03.png)
<br>
**图3**展示了MCTformer-V2的概览。(a) 输入RGB图像。(b) 图像块令牌和附加的类别令牌作为Transformer编码器($L$层)的输入。(c) 编码器输出令牌。(d) 在MCTformer-V2中，引入了一个CAM模块。它由一个卷积层（CONV）和一个全局平均池化（GAP）层组成。它接收Transformer编码器最后一层输出的图像块令牌，并经过重塑（Reshape）后作为输入，输出类别分数（Class scores，即PatchCAM）。(e) 与MCTformer-V1类似，编码器输出的类别令牌也经过平均池化（Average Pooling）产生类别分数。(f) 模型的总损失是这两类类别预测（通过类别令牌和图像块令牌）与图像级真值标签计算出的两个多标签软边际损失之和。(g) 在推理时，融合了类别特定的Transformer注意力（MCT Attention）和PatchCAM图。(h) 从图像块到图像块的Transformer注意力中提取图像块亲和性（Patch Affinity）。(i) 利用图像块亲和性进一步优化融合后的注意力图，产生最终的对象定位图（Refined fusion maps）。
...
**结合PatchCAM和类别特定的Transformer注意力。** 在测试时，可以从最后一个卷积层中提取基于图像块令牌的CAM（此后称为PatchCAM）图。通过对特征图 $F_{out}^{pat}$ 应用min-max归一化，提取PatchCAM图 $A_{pCAM}$，其中 $A_{pCAM} \in \mathbb{R}^{N \times N \times C}$。提取的PatchCAM图随后与所提出的类别特定的Transformer注意力图通过逐元素乘法操作融合，以生成融合的目标定位图 $A$：
